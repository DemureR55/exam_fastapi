...
async def create_user(user: UserCreate, db: AsyncSession = Depends(get_async_db)):
    """
    Регистрирует нового пользователя с ролью 'admin', 'buyer' или 'seller'.
    """
...
async def refresh_token(
    ...
    credentials_exception = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail='Cloud not validate refresh token',
        headers={"WWW-Authenticate": "Bearer"}
    )
...
#почему используем user_id, а не email
...
async def refresh_access_token(body: RefreshTokenRequest,
    ...
    try:
        payload = jwt.decode(old_refresh, SECRET_KEY, algorithms=[ALGORITHM])
        email: int | None = payload.get("sub")
        token_type: str | None = payload.get("token_type")
