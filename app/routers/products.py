...
async def get_all_products(
        ...
        created_at_from: datetime | None = Query(None, description="дата и время создания записи"),
        created_at_to: datetime | None = Query(None, description="дата и время последнего обновления записи"),
    ...
    if created_at_from is not None:
         filters.append(ProductModel.created_at >= created_at_from) #new
    if created_at_to is not None:
         filters.append(ProductModel.created_at <= created_at_to) #new
...
async def get_products_by_category(category_id: int, db: AsyncSession = Depends(get_async_db)):
    ...
    product_result = await db.scalars(select(ProductModel).where(ProductModel.category_id == category_id, ProductModel.is_active == True))
